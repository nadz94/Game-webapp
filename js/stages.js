...
class StageAudioConsent extends Stage {
    constructor(game) {
        ...
        this.selectedIndex = 0; // 0: YES, 1: NO
        this.options = ["YES, ENABLE", "NO, SILENCE"];
    }
    enter() {
        this.game.ui.setMessage("Use Arrows to Select, SPACE/A to Confirm");
        this.game.ui.showBox(false);
        this.game.ui.setHUD("");
    }
    update() {
        if (this.game.input.isJustPressed('ArrowUp')) {
            this.selectedIndex = (this.selectedIndex - 1 + this.options.length) % this.options.length;
        }
        if (this.game.input.isJustPressed('ArrowDown')) {
    ...
    draw(renderer) {
        ...
        for (let i = 0; i < this.options.length; i++) {
            ...
            if (isSelected) {
                renderer.ctx.fillStyle = '#fff';
                renderer.ctx.fillText(`> ${this.options[i]} <`, LOGICAL_W / 2, y);
            } else {
        ...
        renderer.ctx.fillText(this.getPrompt("[ARROWS] TO SELECT, [SPACE] TO CONFIRM", "[ARROWS] TO SELECT, [A] TO CONFIRM"), LOGICAL_W / 2, LOGICAL_H - 20);
